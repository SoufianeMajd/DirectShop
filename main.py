...
app = Flask(__name__)
app.secret_key = 'ef4072ab74fab1912276e53ff09198eb82ed8f0a4f470dfd60d68bb1a596ee12'
...
@app.route("/<sorting>")
def root(sorting):
    loggedIn, firstName, noOfItems, user_type = getUserSessionDetails()
    ...
    if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
        return jsonify({
            ...
            'totalProducts': len(product_rows)
        })

    # Sinon, retourner le template normal
    return render_template(
        "home.html",
        itemData=itemData,
        loggedIn=loggedIn,
...
def getUserSessionDetails():
    with sqlite3.connect('database.db') as conn:
        ...
        if 'email' not in session:
            ...
            loggedIn = True
            cur.execute("SELECT userId, firstName, type FROM users WHERE email = ?", (session['email'],))
            row = cur.fetchone()
            if row:
                userId, firstName, user_type = row
                cur.execute("SELECT count(productId) FROM kart WHERE userId = ?", (userId,))
                ...
                noOfItems = 0
                user_type = None
    return (loggedIn, firstName, noOfItems, user_type)
...
def seller_home():
    ...
    cur.execute("SELECT type FROM users WHERE email = ?", (session['email'],))
    row = cur.fetchone()
    if row is None:
...
def productDescription():
    ...
    with sqlite3.connect('database.db') as conn:
        ...
        vendeur_full_name = f"{vendeur['firstName']} {vendeur['lastName']}" if vendeur else None
        vendeur_email = vendeur["email"] if vendeur else None
...
def register():
    if request.method == 'POST':
        ...
        acceptation = 1          if user_type == 'vendeur' else 1
